...
static counter_t sim_num_single_bubble_q1;
static counter_t sim_num_double_bubble_q1;
static counter_t reg_ready_q1[MD_TOTAL_REGS];
static counter_t sim_num_single_bubble_q2;
static counter_t sim_num_double_bubble_q2;
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "sim_num_single_bubble_q1",
		   "total number of 1 cycle stall (q1)",
			&sim_num_single_bubble_q1, sim_num_single_bubble_q1, NULL);

  stat_reg_counter(sdb, "sim_num_double_bubble_q1",
		   "total number of 2 cycle stall (q1)",
			&sim_num_double_bubble_q1, sim_num_double_bubble_q1, NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   "CPI from RAW hazard (q1)",
		   "1 + (sim_num_single_bubble_q1/sim_num_insn) + (2*sim_num_double_bubble_q1/sim_num_insn)", NULL);
		   
  stat_reg_counter(sdb, "sim_num_RAW_hazard_q2",
		   "total number of RAW hazards (q2)",
		   &sim_num_RAW_hazard_q2, sim_num_RAW_hazard_q2, NULL);
  
  stat_reg_counter(sdb, "sim_num_single_bubble_q2",
		   "total number of 1 cycle stall (q2)",
			&sim_num_single_bubble_q2, sim_num_single_bubble_q2, NULL);

  stat_reg_counter(sdb, "sim_num_double_bubble_q2",
		   "total number of 2 cycle stall (q2)",
			&sim_num_double_bubble_q2, sim_num_double_bubble_q2, NULL);
	   
  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "1 + (sim_num_single_bubble_q2/sim_num_insn) + (2*sim_num_double_bubble_q2/sim_num_insn)" , NULL);
...
#include "machine.def"
    ...
    /* ECE552 Assignment 1 - BEGIN CODE */
      ...
      for (i = 0; i < 3; ++i){
        if(r_in[i] != DNA && reg_ready_q1[r_in[i]] > sim_num_insn){   //Checks for RAW Hazards
          hazard_detected = 1;
          if((reg_ready_q1[r_in[i]] - sim_num_insn) > max_stall){     // To find the biggest stall
            max_stall = (reg_ready_q1[r_in[i]] - sim_num_insn);
          }
        }
      }

      if(hazard_detected){											  // Updating the counters
        if(max_stall == 1){
          sim_num_single_bubble_q1++;
    ...
    sim_num_RAW_hazard_q1 = sim_num_single_bubble_q1 + sim_num_double_bubble_q1;
    /*************** Question 1 ***************/    
    ...
    /*************** Question 2 ***************/
    {
      ...
      for (i = 0; i < 3; ++i){
        if(r_in[i] != DNA && reg_ready_q2[r_in[i]] > sim_num_insn){ 							//Checks for RAW Hazards
          if ((i == 0) && (MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op) & F_STORE)) {			//Checks if Bypassing possible
            continue;
          }
          hazard_detected = 1;
          if((reg_ready_q2[r_in[i]] - sim_num_insn) > max_stall){							    // To find the biggest stall
            max_stall = (reg_ready_q2[r_in[i]] - sim_num_insn);
          }
        }
      }

      if(hazard_detected){																		// Updating counters
        if(max_stall == 1){
          sim_num_single_bubble_q2++;
		  for (int i = 0; i < MD_TOTAL_REGS; i++) {
			reg_ready_q2[i]-=1;
		  }
        }
        else if(max_stall == 2){
          sim_num_double_bubble_q2++;
          for (int i = 0; i < MD_TOTAL_REGS; i++) {
			reg_ready_q2[i]-=2;
		  }
        }
    ...
    sim_num_RAW_hazard_q2 = sim_num_single_bubble_q2 + sim_num_double_bubble_q2;
    /*************** Question 2 ***************/
