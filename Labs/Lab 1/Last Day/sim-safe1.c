...
static counter_t stalls_single_cycle_q1 = 0;
static counter_t stalls_double_cycle_q1 = 0;
static counter_t stalls_single_cycle_q2 = 0;
static counter_t stalls_double_cycle_q2 = 0;
static counter_t total_stalls_q1 = 0;
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   ...
		   "1 + total_stalls_q1 / sim_num_insn", NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "1 + stalls_single_cycle_q2 / sim_num_insn  +  stalls_double_cycle_q2 * 2 / sim_num_insn", NULL);
		   
  stat_reg_counter(sdb, "total_stallq1",
		   "total stall cycles caused by RAW hazards (q1)",
		   &total_stalls_q1, total_stalls_q1, NULL);
		   
  stat_reg_counter(sdb, "sim_num_insn",
		   "sim_num_insn",
		   &sim_num_insn, sim_num_insn, NULL);
		   
  stat_reg_counter(sdb, "stalls_single_cycle_q1",
		   "stalls_single_cycle_q1",
		   &stalls_single_cycle_q1, stalls_single_cycle_q1, NULL);
		   
  stat_reg_counter(sdb, "stalls_double_cycle_q1",
		   "stalls_double_cycle_q1",
		   &stalls_double_cycle_q1, stalls_double_cycle_q1, NULL);
...
/* ECE552 Assignment 1 - BEGIN CODE */
	  ...
	  for (i = 0; i < 3; i++) {
	 	 if (r_in[i] != DNA && reg_ready [r_in [i]] > sim_num_insn) {
			...
			if((reg_ready [r_in [i]] - sim_num_insn) > max_stall) {
				
				max_stall = (reg_ready [r_in [i]] - sim_num_insn);
			}
	 	 }
	  }
	  ...
	  if (max_stall == 1) {
		...
		total_stalls_q1 = total_stalls_q1 + stalls_single_cycle_q1;
	  }
	  else if (max_stall == 2) {
		stalls_double_cycle_q1++;
		sim_num_RAW_hazard_q1++;
		total_stalls_q1 = total_stalls_q1 + (stalls_double_cycle_q1 * 2);
	  }
