...
static counter_t sim_num_one_bubble_q1;
static counter_t sim_num_two_bubble_q1;
static counter_t sim_num_one_bubble_q2;
static counter_t sim_num_two_bubble_q2;
...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   ...
		   "1 + (sim_num_one_bubble_q1/sim_num_insn) + ((sim_num_two_bubble_q1/sim_num_insn) * 2)" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "1 + (sim_num_one_bubble_q2/sim_num_insn) + ((sim_num_two_bubble_q2/sim_num_insn) * 2)" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);
...
/* ECE552 Assignment 1 - STATS COUNTERS - BEGIN */
	  int counter_q1=0, counter_q2=0, cycle=0;
	  
	  for (i = 0; i < 3; i++) {
		  if (r_out[0] != DNA && reg_ready_q1[r_in[i]] > sim_num_insn ) {
			cycle = reg_ready_q1[r_in[i]] - sim_num_insn;
