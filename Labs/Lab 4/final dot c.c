	     ...
	     int prefetch_type)		/* prefetcher type */
{
  ...
  /* ECE552 Assignment 4 - BEGIN CODE*/
  //Initialization for RPT
  //States- 1-Initial, 0-Steady, 2-Transient, 3-No-prediction 
  if (prefetch_type > 2) {
	 int rpt_size = prefetch_type;
	 cp->rpt = calloc(rpt_size, sizeof(struct reference_predictor_table)); 
	 for (int i=0;i<rpt_size;i++) {
		...
		cp->dcpt = calloc(512, sizeof(struct delta_correlating_prediction_table));
  /* ECE552 Assignment 4 - END CODE*/
...
void next_line_prefetcher(struct cache_t *cp, md_addr_t addr) {
	...
	/* ECE552 Assignment 4 - BEGIN CODE*/
	md_addr_t next_block_address = addr + cp->bsize;
	md_addr_t next_block_address_tag = CACHE_TAG(cp,next_block_address);
	md_addr_t next_block_address_set = CACHE_SET(cp,next_block_address);
	md_addr_t next_block_prefechted_address = CACHE_MK_BADDR(cp, next_block_address_tag, next_block_address_set);
	int Address_in_cache = cache_probe(cp,	
	    next_block_prefechted_address);
	if( Address_in_cache == FALSE)//block not in cache
	{
		cache_access(cp,	/* cache to access */
	     Read,		/* access type, Read or Write */
	     next_block_prefechted_address,		/* address of access */
...
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {
	...
	void delta_correlation(struct cache_t *cp, struct delta_correlating_prediction_table *dcpt,
	int ptr, md_addr_t **candidates, int *index) {
		
		int d1 = dcpt->delta[ptr];
		int d2 = dcpt->delta[(ptr - 1 + delta_array) %delta_array];
		md_addr_t addr_local = dcpt->prev_addr;
		md_addr_t last = 0;
		md_addr_t *candidates_local = malloc(delta_array * sizeof(md_addr_t));
		*index = 0;
		
		for (int i = ((ptr - 2 + delta_array) % delta_array); i != ptr; i = ((i - 2 + delta_array ) % delta_array)) {
			//printf("%d\n",ptr);
			//printf("%d---569 \n", i);
			int u = dcpt->delta[i];
			int v = dcpt->delta[(i-1 + delta_array ) % delta_array];
			int x = 0;
			int remaining = 0;
			if ((d1 == u) && (d2 == v)) {
				for (int j = i; j!=ptr; j=(j+1+delta_array ) %delta_array) {
					//printf("576 \n");
					addr_local = addr_local + dcpt->delta[j];
					if(addr_local != last) {
						candidates_local[x] = addr_local;
						last = addr_local;
						x = (x + 1);
				}
					remaining = (j + 1 +delta_array) % delta_array;
				}
				addr_local = addr_local + dcpt->delta[remaining];
				if(addr_local != last) {
					candidates_local[x] = addr_local;
					last = candidates_local[x];
					x = (x + 1 );
				}
	...
	int hashed_index = (tag_pc >> 3) % delta_table;
	int stride = addr - (cp->dcpt)[hashed_index].prev_addr;
	
	//main algorithm start
	
	if ((cp->dcpt)[hashed_index].tag != tag_pc) {
		
		//printf("New Tag\n");
		
		(cp->dcpt)[hashed_index].tag = tag_pc;
		(cp->dcpt)[hashed_index].prev_addr = addr;
		(cp->dcpt)[hashed_index].prev_prefetch = 0;
		
		for (int i=0; i<delta_array; i++)
			(cp->dcpt)[hashed_index].delta[i] = 0;
	}
	
	else if (stride != 0) {
		//printf("ptr----- %d\n",(cp->dcpt)[hashed_index].ptr);
		md_addr_t *candidates = NULL;
		int index;
		(cp->dcpt)[hashed_index].delta[(cp->dcpt)[hashed_index].ptr] = stride;
		(cp->dcpt)[hashed_index].ptr = ((cp->dcpt)[hashed_index].ptr + 1 + delta_array) % delta_array;
		(cp->dcpt)[hashed_index].prev_addr = addr;
		delta_correlation(cp, &(cp->dcpt)[hashed_index], (((cp->dcpt)[hashed_index].ptr - 1 + delta_array) % delta_array), 
		&candidates, &index);
		//printf("returned\n");
		
		if (candidates != NULL  ) {
			//printf("1\n");
			md_addr_t prefetch[32] = {0};
			//printf("2\n");
			int inFlight[32] ={0};
			//printf("3\n");
			int new_index = 0;
			//printf("4\n");
			int prev_prefetch = (cp->dcpt)[hashed_index].prev_prefetch;
			//printf("index--- %d\n", index);
			//printf("5\n");
			for (int i = 0 ; i < index; i++) {
				...
				if(cache_probe(cp, candidates[i])) {
					//printf("incache----  %d\n", candidates[i]);
					in_cache = 1;
				}
				//printf("------------------------------------------\n");
				...
				if ((!in_flight) && (!in_cache)) {
					//printf("in_flight and in_cache passed---- %d", candidates[i]);
					prefetch[new_index] = candidates[i];
					new_index = (new_index + 1 );
					(cp->dcpt)[hashed_index].prev_prefetch = candidates[i];
			...
			if (new_index != 0) {
				//printf("Prefetch start---- %d\n",new_index);
				for (int i = 0; i < new_index; i++) {
					//printf("663###########--Prefetched \n");
...
void stride_prefetcher(struct cache_t *cp, md_addr_t addr) {
	...
	void access_cache(md_addr_t index, md_addr_t addr) {
		if(!cache_probe(cp, (addr + (cp->rpt[index]).stride))){
			cache_access(cp, Read, CACHE_BADDR(cp, (addr + (cp->rpt)[index].stride)), NULL, cp->bsize, 0, NULL, NULL, 1);
		}
	}

	int stride_finder(md_addr_t index, md_addr_t addr) {
		int stride = addr - cp->rpt[index].prev_addr;
		return stride;
	}
	
	int rpt_size = cp->prefetch_type;
	int tag_pc = get_PC();
	int hashed_index = (tag_pc >> 3) % rpt_size;
		
	if ((cp->rpt)[hashed_index].tag != tag_pc) {    //Senario 1
		
		(cp->rpt)[hashed_index].tag = tag_pc;
		(cp->rpt)[hashed_index].prev_addr = addr;
		(cp->rpt)[hashed_index].stride = 0;
		(cp->rpt)[hashed_index].state = 1;
	}
	
	else {                                          //Senario 2
		int new_stride = stride_finder(hashed_index, addr);
		int stride_result;
		if ((cp->rpt)[hashed_index].stride == new_stride)
			stride_result = 1;
		else
			stride_result = 0;
		
		if (stride_result) {
			switch ((cp->rpt)[hashed_index].state) {
				
				case 0:
				(cp->rpt)[hashed_index].state = 0;
				access_cache(hashed_index, addr);
				break;				
			...
			switch ((cp->rpt)[hashed_index].state) {
				...
				case 0:
				(cp->rpt)[hashed_index].state = 1;
				//(cp->rpt)[hashed_index].stride = new_stride;
				access_cache(hashed_index, addr);
				break;				
...
//md_addr_t get_PC();   //Commented this
