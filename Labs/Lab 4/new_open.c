...
//552
	...
	if (cp->prefetch_type == 2) { 
		//printf("initing open ended vars\n\n\n\n");
		cp->cache_miss_list = (md_addr_t*) calloc(6144, sizeof(md_addr_t));   
		cp->cache_miss_count = 0;
		//printf("cp->cache_miss_count=%d\n",cp->cache_miss_count);

		for (int k =0; k < 6144; k++){
		  cp->cache_miss_list[k] = 0;
		}
	}
...
void open_ended_prefetcher(struct cache_t *cp, md_addr_t addr) {
	//printf("\n");
	int stride_open_prefetch(struct cache_t *cp, md_addr_t addr, md_addr_t tag_pc, int hashed_index) {
	...
	//States- 1-Initial, 0-Steady, 2-Transient, 3-No-prediction
		...
		if (cp->rpt[hashed_index].tag != tag_pc) {    //Senario 1
			...
			//printf("assigning new entry at %d;;;", hashed_index);
			return 0;
		}
			...
			cp->rpt[hashed_index].prev_addr = addr;
			//printf("existing entry at %d, made stride=%d, state=%d;;;", hashed_index, cp->rpt[hashed_index].stride, cp->rpt[hashed_index].state);
	...
	//	printf("allocating cache");		
	  ...
	  md_addr_t table_index = (tag>>pc_offset) % rpt_size;

	  int stride_prefetch_tf = stride_open_prefetch(cp, addr, get_PC(), table_index);
	  if(stride_prefetch_tf && cp->rpt[table_index].state == 0){
		if(!cache_probe(cp, (addr + cp->rpt[table_index].stride))){
		//	  printf("calling cache_Access1 for addr=%h",(addr + cp->rpt[table_index].stride));

		  cache_access(cp, Read, CACHE_BADDR(cp, (addr + cp->rpt[table_index].stride)), NULL, cp->bsize, 0, NULL, NULL, 1);
		}
	  }
	  else{
		for(i = 0; i < 6144; i ++){
		  if(cp->cache_miss_list[i] == addr){
			md_addr_t next_possible_miss = cp->cache_miss_list[(i+1)%6144];
			if(next_possible_miss != 0 && !cache_probe(cp, next_possible_miss)){
			 // printf("calling cache_Access2 for addr=%h",next_possible_miss);
			  cache_access(cp, Read, CACHE_BADDR(cp, next_possible_miss), NULL, cp->bsize, 0, NULL, NULL, 1);
			}
			break;
		  }
		}      
	     ...
	     int prefetch)		/* 1 if the access is a prefetch, 0 if it is not */
{
 ...
 // if (prefetch) printf("entering cache_Access");
  ...
  if (prefetch == 0 ) {
	...
	cp->read_misses++;
	 ...
	 /* ECE552 Assignment 4 - BEGIN CODE*/
    //Insert Miss address to list
	if (cp->prefetch_type == 2) { 
      cp->cache_miss_list[cp->cache_miss_count%6144] = addr;
	  //printf("cp->cache_miss_count=%d\n",cp->cache_miss_count);
      cp->cache_miss_count = (cp->cache_miss_count + 1) % 6144;
    }
    /* ECE552 Assignment 4 - END CODE*/
 ...
 cache_fast_hit: /* fast hit handler */
  ...
  //if (prefetch) printf("exiting cache_Access");
